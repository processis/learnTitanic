...
接下来，我讲述了60年代美国费城的Weisbord先生针对印刷公司订单管理部使用XY理论改革团队的故事：如何从每一步骤都要按小主管(supervisor)安排的传统管理模式（X型管理），变为由4-5人自我管理的独立团队（Y型管理），最终把每天处理订单量提升50\%的成功改革故事。（详见第2章故事）\\
...
冲刺回顾本应是团队分析根因在下一轮冲刺改进的最佳时机，但很多团队不了解改正(Correction)和
纠正措施(Corrective
Action)的区别（详见之前第9章），团队在回顾时只讨论改正，导致以前发生过的问题还是会再发生。
...
``要做好PDCA背后就要分析根本原因，讨论纠正措施。其实不需要高深的统计分析，只须使用二八原则识别改进方向，然后大家头脑风暴，比如使用鱼骨图记录主要根因，讨论纠正措施，制定计划，最后判断是否得到显著提升就可以了。你觉得开发团队在能否回顾时用上？''\\
``听你这样说应该不难。''\\
``其实甚至一家日本俱乐部里由7位年轻女服务员组成的QC圈在84年已经能做到。如果不信，可以发你相关分享文章。''\\
...
可参考附件
A:``绅士俱乐部过程改进''了解团队如何按以上步骤做过程改进。\strut
\end{minipage}}

但不要误以为只要使用各种根因分析方法，如帕累托图、鱼骨图等，就能做好根因分析。以上的根因分析都是以定性讨论为止，没有利用相关数据做分析。
没有数据，就无法谈改进, 要做好根本原因分析也必须利用数据。
如收集了缺陷与返工工作量数据，并利用二八原则便能找出应该针对哪方面入手，
而不是指靠个人的主观判断和经验，直接制定改进措施。

应收集哪些数据？数据不是越多越好，因为收集数据花精力，所以是反过来越少越好。应针对收集哪些数据呢？就应考虑改进目标，例如按上章想降低返工，便应问以下问题：

\includegraphics[width=10cm]{Screenshotfrom2024-07-2422-56-47.png}
...
\includegraphics[width=10cm]{Jalote_emm_71_10.png}
...
\includegraphics[width=10cm]{198页替换图片.jpg}
...
如果我们按上面各阶段缺陷利率百分比，设计与需求排除缺陷15\%，代码评审与单元测试55\%，集成/系统测试22\%，验收测试8\%，下面按缺陷总数为100，
得出算出设计与需求排除共15缺陷, DR+RR=15，CR+UT=55, IT+ST=22,
AT=8,求出各阶段的缺陷输入与缺陷排除率，把参数输入预测模型(XLS 模板）。\\
得出下面的表，缺陷分布是中间最高头尾低，右面与左面不同，有条长尾巴，类似估算软件开发项目工作量的Rayleigh曲线。

\includegraphics[width=10cm]{Jalote_emm_72-1.jpg}
...
组长：有，What When ..\\
我：不，你说的是5W+1H
，5W是五个为什么(Why)，你们应该不断追问``为什么''，才能更好地识别出背后的主因，并针对主因采取纠正措施，避免同类问题重复发生。\\
接着，我向他们讲述了丰田汽车大野耐一先生的5W故事（详见第一章附件``5Why例子''）。\\
除了通过问``为什么''之外，还可以通过画整个流程，识别有哪些失效点。\\
但5W只算是定性分析，简单介绍几种依据数据的量化根因分析方法：

\begin{itemize}
\tightlist
\item
  分析全过程(Process)
  例如，用FMEA分析搭不上飞机的根因为例子（详见附件），先画出整个过程，如果是软件开发，可以从需求开始到最终交付的全流程，然后按历史数据，分析哪个环节的影响交付的质量，并估计影响度、发生概率等参数，针对高风险的``失效''分析根因
\item
  也可以从时间点的角度，使用控制图，从时间顺序查看有没有异常点。
  后面24章会详细介绍控制图\\
\end{itemize}
...
如想多了解应如何分析与对应各类与人员相关的缺陷，请看除了要附件里``如何分析与对应各类由于人员引起的缺陷''。\\
\end{description}

当敏捷开发团队除了要用各种方法分析根因时，还应考虑让各团队成员全心投入参与。只有这样，过程改进才能持久。下章会举例说明。\\
...
\hline
1&某些缺陷，发生模式是随机的，不容易看出哪类缺陷经常发生。&错误是由于疏忽造成的。&完善过程，防止人为错误。\\
\hline
2&某些缺陷，有些人员经常犯错，但另一些人员总是“很好”。&错误是由于缺乏技巧造成(能力、知识、技能等)。例如某些窍门只有部分人知道。&挖掘和推广窍门，避免出现小众才知道的秘密。\\
\hline
3&有些人员总是在各种各样的缺陷上容易犯错。&潜在的原因：故意不遵守标准。天生不能完成这类任务。缺乏培训。&激励措施。换人。提供培训。\\
\hline
4&某些缺陷，所有工人都容易犯错。&缺陷是应该由管理层控制。&语言标准化；提供翻译，词汇表。达到自我控制标准。\\
...
\tightlist
\item
  本文里的来福枪故事是上面第3类的标准例子
\item
  例如，下图统计了6位技术人员，按29种缺陷统计：
\end{itemize}

\href{文件:Juran_t5.8.jpg}{文件:Juran t5.8.jpg}

分析：\\
*虽然第3类缺陷出现共19次，最多，但大部分是源自员工乙，但他除了犯第三类缺陷外，他其他方面的缺陷数很少。很可能是他误解了里面某些关键过程（上表的第二类）
