...
library(tidymodels)
tidymodels_prefer()
tbDesharnais <- read_csv("cleanDesharnais74.csv",col_names = TRUE,na = '-1',
                         cols(
                           'Project' = col_integer(),
                           'TeamExp' = col_integer(),
...
desharnais <- read.table(desharnais123fillMedLog71TrainSet-loglength.csv",
                         sep = ",", header = TRUE)
...
deshaTrain = deshaTrain[,c(3:4,14,15,16,17,18,19)]

deshaTest = deshaTest[,c(3:4,14,15,16,17,18,19)]
...
deshaTrainX = deshaTrain[,c(1:6,8)]
deshaTestX = deshaTest[,c(1:6,8)]
