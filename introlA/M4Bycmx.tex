...
（注：MMRE = Mean Magnitude of Relative Error, MedMRE = Median MRE , Pred(.25)=偏差少于0.25之比例）\\
...
\subsubsection{建立新变量Creation of New
...
\subsubsection{用直方图(Histograms)初步分析}\label{histogramsux7528ux76f4ux65b9ux56feux521dux6b65ux5206ux6790}}
...
\tightlist
\item
  少于α (通常是0.05)的p-值表示统计上有显著关系
\end{itemize}

\begin{enumerate}
\tightlist
\item
  使用方差分析法(ANOVA)分析整个模型是否具备显著关系，分析报告包括F-检验结果与相关p-值
\item
...
\hypertarget{row-pruning}{%
\subsubsection{例子}\label{row-pruning}}

想确定在为每一个特定产品分配的货架空间的数量和对同样的产品的销售量之间是否存在关系。她调查同样链的30个不同的商店，所有的都有相似的人口统计资料。比较分配的货架空间（x）和销售（y）收集数据。使用一个计算机程序来运行方差分析显示了下面的结果：
