...
历史的平均值是285，得出检验统计量值为2.06(计算方程式详见附件) 超出了95%置信区间土1.96范围,所以我们拒绝零假设,样本与285有显著差异（提升）\\
...
\tightlist
\item
  实际情况是\(\mathbf{H}_0\)成立,从检验结果选择了\(\mathbf{H}_1\)不成立,即拒绝了\(\mathbf{H}_0\)，这时称该检验犯了第一类错误(Type
  I error)或``弃真''错误
\item
  实际情况是\(\mathbf{H}_0\)不成立,\(\mathbf{H}_1\)成立,而从检验结果选择了\(\mathbf{H}_0\),这时犯了第二类错误(Type II error),或称“取伪”错误
\end{enumerate}
