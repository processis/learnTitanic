...
历史的平均值是285，并考虑样本平均值，样本方差，样本数量得出
检验统计量计算值\(Z_n\)为 2.06 （计算方程式详见附件） 超出了95\%置信区间
...
要了解P值是什么？为什么低于0.05可以拒绝？ 首先要理解会发生两类错误。
...
\tightlist
\item
  实际情况是\(\mathbf{H}_0\)成立,从检验结果选择了\(\mathbf{H}_a\)不成立,即拒绝了\(\mathbf{H}_0\)，犯了第一类错误(Type
  I error)或``弃真''错误
\item
  实际情况是\(\mathbf{H}_0\)不成立,\(\mathbf{H}_1\)成立,而从检验结果选择了\(\mathbf{H}_0\),犯了第二类错误(Type II error),或称“取伪”错误
\end{enumerate}
...
两组50个数之间没有对应关系），设定临界概率为5% （Alpha \(\alpha\)=0.05），是否有足够的证据支持这种说法? （假设方差都一样， \(\sigma\) = 3.3）。
...
假定使用90\%置信区间:\\
正态分布 \(\pm 1.65{\sigma}\) (标准差)内的面积是总面积的90\%，计算出检验统计量计算值为1.06， 从下图看到 1.06 落在90%置信区间之内，所以不能拒绝零假设。\\
...
可以使用相关方程式，计算出 t值 =1.61 （计算过程，详见附件）
...
用Tukey-Kramer 相关方程式（详见附件），得出临界范围为 4.47，所以判断只有供货商1与2之间的布料拉力之间是有显著差异。 
...
接近正态分布。\\
%\href{文件:!NormalDistributionBlankScreenshot_2023-04-16_201559.jpg}{600px}
...
男生：\(\overline{X_1} = 8.6\) ； \(\overline{\sigma_1} = 3.3\)\\
